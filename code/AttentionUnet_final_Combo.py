...
batch_size = 8
num_epoch = 100
os.chdir('/home/ubuntu/Ohseok/EUS_seg')
data_dir = './datasets/segmentation_final6'
ckpt_dir = './AttentionUnet_seg_final_Combo/checkpoint'
log_dir = './AttentionUnet_seg_final_Combo/log'
result_dir = './AttentionUnet_seg_final_Combo/result'
mode = 'train'
train_continue = 'off'
kf_type = True
...
externalset = os.listdir(os.path.join(data_dir, 'external'))
externalset = [i for i in externalset  if 'label' in i]
...
def save_best_npy(FOLD):
    ...
    loader_test = DataLoader(dataset_test, batch_size=batch_size, shuffle=False, num_workers=0)
    print(os.path.join(data_dir, 'fold' + str(FOLD), 'test'))
...
if mode == 'train':
    ...
    for fold in range(4,5):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, mode='min', patience=10,factor=0.5,min_lr=1e-6, verbose=True)
